...
$('#cep').on('blur',function(event){if(document.querySelector("#cep").value.length==9){event.preventDefault();CarregandoLoadingSrv();var dados=$(this).serialize();$.ajax({url:getRoot(urlSrvDetalhesCep),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){inputsAddresSrv(response.inputs)
ParandoLoadingSrv()}else{M.toast({html:`<span class="lighten-2">${response.erros}</span><button class="btn-flat toast-action"><i class="material-icons yellow-text">error_outline</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,});clearInputsAddresSrv();ParandoLoadingSrv()}}})}});$(document).ready(function(){$('select').formSelect()})