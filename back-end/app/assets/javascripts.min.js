...
return output.join("").substr(0,i)};VMasker.toNumber=function(value){return value.toString().replace(/(?!^-)[^0-9]/g,"")};VMasker.toAlphaNumeric=function(value){return value.toString().replace(/[^a-z0-9 ]+/i,"")};return VMasker}));const urlName='cars7.1';const urlCadastro=urlName+'/cadastro';const urlLogin=urlName+'/srv/login';const urlRedefinePassword=urlName+'/srv/redefinir_senha';const urlNewPassword=urlName+'/srv/nova_senha';function getRoot(url){var root="http://"+document.location.hostname+"/"+url;return root}
