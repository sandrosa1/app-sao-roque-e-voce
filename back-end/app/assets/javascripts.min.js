...
var dados=$(this).serialize();$.ajax({url:getRoot(UrlSrvConfig),type:'post',dataType:'json',data:dados,success:function(response){$('.loading').empty();if(response.retorno=='erro'){getCaptcha();$.each(response.erros,function(key,value){M.toast({html:`<span class="lighten-2">${value}</span><button class="btn-flat toast-action"><i class="material-icons yellow-text">error_outline</i></button>`,classes:'rounded, red',inDuration:5000,outDuration:5000,})})}else{$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action"><i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded, green',inDuration:5000,outDuration:5000,})});setTimeout(()=>{window.onload},3000)}
BreaKLoadingConfig()}})})