...
return output.join("").substr(0,i)};VMasker.toNumber=function(value){return value.toString().replace(/(?!^-)[^0-9]/g,"")};VMasker.toAlphaNumeric=function(value){return value.toString().replace(/[^a-z0-9 ]+/i,"")};return VMasker}));const urlName='app-sao-roque-e-voce/back-end';const urlCadastro=urlName+'/cadastro';const urlLogin=urlName+'/srv/login';const urlRedefinePassword=urlName+'/srv/redefinir_senha';const urlNewPassword=urlName+'/srv/nova_senha';const urlLoginRacs=urlName+'/racs/login';function getRoot(url){var root="http://"+document.location.hostname+"/"+url;return root}
const gif=getRoot(urlName+'/img/anime.gif')
const logoSrv=getRoot(urlName+'/img/logo-srv-200.png')
const CarregandoLoading=()=>document.getElementById('root').innerHTML=`<img class="gif" src='${gif}' alt="Logomarca da WEF">`;const ParandoLoading=()=>document.getElementById('root').innerHTML=`<img  src='${logoSrv}' alt="Logomarca da WEF">`;$('#cpf , #birthDate, #phone').on('focus',function(){var id=$(this).attr("id");if(id=="cpf"){VMasker(document.querySelector("#cpf")).maskPattern("999.999.999-99")}
if(id=="birthDate"){VMasker(document.querySelector("#birthDate")).maskPattern("99/99/9999")};if(id=="phone"){VMasker(document.querySelector("#phone")).maskPattern("(99)-999999999")}});function getCaptcha(){grecaptcha.ready(function(){grecaptcha.execute(sitekey,{action:'homepage'}).then(function(token){const gRecaptchaResponse=document.querySelector("#g-recaptcha-response").value=token})})}
getCaptcha();$("#formCadastro").on("submit",function(event){event.preventDefault();CarregandoLoading()
var dados=$(this).serialize();$.ajax({url:getRoot(urlCadastro),type:'post',dataType:'json',data:dados,success:function(response){$('.retornoCad').empty();if(response.retorno=='erro'){getCaptcha();$.each(response.erros,function(key,value){M.toast({html:`<span class="lighten-2">${value}</span><button class="btn-flat toast-action"><i class="material-icons yellow-text">error_outline</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})})}else{$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action"><i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})});setTimeout(()=>{window.location.href=response.page},3000)}
ParandoLoading()}})});$("#senha").keypress(function(e){kc=e.keyCode?e.keyCode:e.which;sk=e.shiftKey?e.shiftKey:((kc==16)?true:!1);if(((kc>=65&&kc<=90)&&!sk)||(kc>=97&&kc<=122)&&sk){$(".resultadoForm").html("Caps Lock Ligado")}else{$(".resultadoForm").empty()}});$("#formLogin").on("submit",function(event){CarregandoLoading()
event.preventDefault();var dados=$(this).serialize();$.ajax({url:getRoot(urlLogin),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})});$("#formRedefinePassword").on("submit",function(event){event.preventDefault();CarregandoLoading()
var dados=$(this).serialize();$.ajax({url:getRoot(urlRedefinePassword),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action"><i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})});window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})});$("#formNewPassword").on("submit",function(event){event.preventDefault();CarregandoLoading()
var dados=$(this).serialize();$.ajax({url:getRoot(urlNewPassword),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action">
                <i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})});window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})});$("#formLoginRacs").on("submit",function(event){CarregandoLoading()
event.preventDefault();var dados=$(this).serialize();$.ajax({url:getRoot(urlLoginRacs),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})});const UrlSrvConfigCep=urlName+'/srv/configuracao/cep';const UrlSrvConfig=urlName+'/srv/configuracao';const UrlSrvDetalhes=urlName+'/srv/detalhes';const UrlSrvTela=urlName+'/srv/tela';const LoadingCep=()=>document.getElementById('loadingCep').innerHTML='<div class="preloader-wrapper small active"><div class="spinner-layer spinner-green-only"><div class="circle-clipper left"><div class="circle"></div></div><div class="gap-patch"><div class="circle"></div></div><div class="circle-clipper right"><div class="circle"></div></div></div></div>';const ParandoLoadingCep=()=>document.getElementById('loadingCep').innerHTML='';const LoadingConfig=()=>document.getElementById('loading').innerHTML='<div class="preloader-wrapper small active"><div class="spinner-layer spinner-green-only"><div class="circle-clipper left"><div class="circle"></div></div><div class="gap-patch"><div class="circle"></div></div><div class="circle-clipper right"><div class="circle"></div></div></div></div>';const BreakLoadingConfig=()=>document.getElementById('loading').innerHTML='';$(document).ready(function(){$('.modal').modal()});$('#cep, #telefone, #celular').on('focus',function(){var id=$(this).attr("id");if(id=="cep"){VMasker(document.querySelector("#cep")).maskPattern("99999-999")};if(id=="telefone"){VMasker(document.querySelector("#telefone")).maskPattern("(99)9999-9999")};if(id=="celular"){VMasker(document.querySelector("#celular")).maskPattern("(99)99999-9999")}});const inputsAddresSrv=(result)=>{for(const campo in result){if(document.querySelector("#"+campo)){document.querySelector("#"+campo).value=result[campo]}}}
const clearInputsAddresSrv=()=>{document.querySelector("#cep").value='';document.querySelector("#logradouro").value='';document.querySelector("#bairro").value='';document.querySelector("#localidade").value=''}
$('#cep').on('blur',function(event){if(document.querySelector("#cep").value.length==9){event.preventDefault();LoadingCep();var dados=$(this).serialize();$.ajax({url:getRoot(UrlSrvConfigCep),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){inputsAddresSrv(response.inputs)
ParandoLoadingCep()}else{M.toast({html:`<span class="lighten-2">${response.erros}</span><button class="btn-flat toast-action"><i class="material-icons yellow-text">error_outline</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,});clearInputsAddresSrv();ParandoLoadingCep()}}})}});$("#formConfig").on("submit",function(event){event.preventDefault();LoadingConfig()
...
BreaKLoadingConfig()}})});$('#semana, #sabado, #domingo, #feriado').on('focus',function(){var id=$(this).attr("id");if(id=="semana"){VMasker(document.querySelector("#semana")).maskPattern("99:99 - 99:99")};if(id=="sabado"){VMasker(document.querySelector("#sabado")).maskPattern("99:99 - 99:99")};if(id=="domingo"){VMasker(document.querySelector("#domingo")).maskPattern("99:99 - 99:99")};if(id=="feriado"){VMasker(document.querySelector("#feriado")).maskPattern("99:99 - 99:99")}});$("#formDetahes").on("submit",function(event){event.preventDefault();LoadingConfig()
