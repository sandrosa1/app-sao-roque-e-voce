...
if(id=="birthDate"){VMasker(document.querySelector("#birthDate")).maskPattern("99/99/9999")};if(id=="phone"){VMasker(document.querySelector("#phone")).maskPattern("(99)-999999999")}});function getCaptcha(){grecaptcha.ready(function(){grecaptcha.execute('6LfHswQdAAAAAPuerl_eJ6d2NPw7Ognc5OaIwNE4',{action:'homepage'}).then(function(token){const gRecaptchaResponse=document.querySelector("#g-recaptcha-response").value=token})})}
...
event.preventDefault();var dados=$(this).serialize();$.ajax({url:getRoot(urlLogin),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){$('.loginFormulario').hide()}
...
var dados=$(this).serialize();$.ajax({url:getRoot(urlRedefinePassword),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){$('.loginFormulario').hide();$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action"><i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})})}else{getCaptcha();if(response.tentativas==!0){$('.loginFormulario').hide()}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
...
var dados=$(this).serialize();$.ajax({url:getRoot(urlNewPassword),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){$.each(response.success,function(key,value){M.toast({html:`<span>${value}</span><button class="btn-flat toast-action">
                <i class="material-icons green-text">thumb_up</i></button>`,classes:'rounded',inDuration:5000,outDuration:5000,})});window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){$('.loginFormulario').hide()}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})});$("#formLoginRacs").on("submit",function(event){CarregandoLoading()
event.preventDefault();var dados=$(this).serialize();console.log(dados)
$.ajax({url:getRoot(urlLoginRcas),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){$('.loginFormulario').hide()}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})})