...
event.preventDefault();var dados=$(this).serialize();$.ajax({url:getRoot(urlLoginRacs),type:'post',dataType:'json',data:dados,success:function(response){if(response.retorno=='success'){window.location.href=response.page}else{getCaptcha();if(response.tentativas==!0){$('.loginFormulario').hide()}
$('.resultadoForm').empty();$.each(response.erros,function(key,value){$('.resultadoForm').append(value+'<br>')})}
ParandoLoading()}})})